...
public class ServletAddProduct extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (ps.insertProduct(id, name, priceInt, quantity, material, size, color, gender, idCategory) > 0) {
            ...
            StringTokenizer stk = new StringTokenizer(root, File.separator);
            String pathSave = "";
            while (stk.countTokens() > 2) {
                pathSave+=stk.nextToken() + "/";
            }
            pathSave+="src\\main\\webapp\\assets\\images\\product_img\\";
            String path = "./assets/images/product_img/";
            for (Part part : parts) {
                String fileName = part.getSubmittedFileName();
                if (fileName != null) {
                    part.write(pathSave + fileName);
